...
* Summary of set up:
	...
	The code depends on some auxiliary libraries in ./Library  and ./Definition_Error_Model subdirectories. You will notice that you need to generate these libraries before making 'make MODEL=SEI1I2AAdYR_AGE_MPOP'. A recursive makefile does this job for you. However, if gcc does not find these libraries, it means they have been accidentally deleted and you should build them bakc up again. Also, the code is linked against R libraries.  You may remove these R links or install R in your system. I recommend this 2n option. This will allow you to create shared libraries that, then, can be called as standard R funcions from RStudio, for example. 
